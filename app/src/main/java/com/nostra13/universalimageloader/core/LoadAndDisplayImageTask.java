...
import com.nostra13.universalimageloader.utils.NetTaskCountManager;
	...
	public void run() {
		...
		try {
			...
			if (bmp == null || bmp.isRecycled()) {
				bmp = tryLoadBitmap();		//从内存卡和网路网络中获取图片
				if (bmp == null) {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				...
				NetTaskCountManager.getInstance().addTask();
				if (options.isCacheOnDisk() && tryCacheImageOnDisk()) {		//这里从网路获取数据
				...
				NetTaskCountManager.getInstance().removeTask();
				if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
