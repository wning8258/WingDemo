...
public class SceneProxy implements ISignt {
	...
	private Matrix mMatrix;//开始动画matrix
	private Matrix mEndMatrix;//结束动画matrix
	...
	public void play(Canvas canvas) {
		final int timeDifference = (int) (SystemClock.uptimeMillis() - mBeginTime);
       // LogUtils.i("wn","timeDifference : "+timeDifference+", mSumTime : "+mSumTime);
		...
		if (timeDifference <= mBeginLastPoint && mBeginAnimation != null) {
			...
			mEndAnimation.execute(mEndMatrix, mPaint, timeDifference-mEndLastPoint);
			mMatrix.setConcat(mMatrix,mEndMatrix);
		}
...
//		canvas.drawRect(test_rectf, test_paint);
	...
	public void setmBeginAnimation(AnimationWrapper mBeginAnimation,int duration) {
		this.mBeginAnimation = mBeginAnimation;
		mBeginAnimationDuration=duration+200;//为了防止误差加上100的偏移
	}
