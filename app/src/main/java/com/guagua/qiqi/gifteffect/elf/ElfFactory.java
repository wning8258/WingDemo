...
import com.guagua.qiqi.gifteffect.animation.algorithm.Decay2Stop;
...
import com.guagua.qiqi.gifteffect.animation.algorithm.RightValue;
...
//		paintShape.setStartOffset(MathCommonAlg.rangeRandom(0000, 3000));
	...
	*/
	 ...
	 * @return: void
	*/
	public static void endowLevel7StarYun(Element bitmapShape, Rect rect, Context context, boolean direction,int startOffset) {
		//背景的宽度和高度
		final int width = rect.width();
		final int height = rect.height();
		final float valueF = MathCommonAlg.randomFloat(0, 1);
		...
		if (direction) {
			if (bool) {
				caculateCommonHandle.c_x = Decay2.build(-width / 10f * MathCommonAlg.randomFloat(1, 2), MathCommonAlg.randomFloat(0, 1.5f),
						rect.left + width * valueF);//StaticValue.build(MathCommonAlg.randomFloat(rect.left,rect.right-circleShape.getRadius()));
			}
			else {
				caculateCommonHandle.c_x = DecayOpposite2.build(-MathCommonAlg.randomFloat(0.1f, 0.5f), rect.left + width * valueF);
			}
		}
		else {
			if (bool) {
				caculateCommonHandle.c_x = Decay2.build(width / 10f * MathCommonAlg.randomFloat(1, 2), MathCommonAlg.randomFloat(0, 1.5f),
						rect.right - width * valueF);//StaticValue.build(MathCommonAlg.randomFloat(rect.left,rect.right-circleShape.getRadius()));
			}
			else {
				caculateCommonHandle.c_x = DecayOpposite2.build(MathCommonAlg.randomFloat(0.1f, 0.5f), rect.right - width * valueF);
			}
		}

		caculateCommonHandle.c_y = StandarCal.build(MathCommonAlg.rangeRandom(rect.top, rect.bottom),
				height * MathCommonAlg.randomFloat(0.2f, .8f));//StaticValue.build(MathCommonAlg.randomFloat(rect.top, rect.bottom-circleShape.getRadius()));
		caculateCommonHandle.c_rotation = StaticValue.build(0);
		caculateCommonHandle.c_scale = StandarCal.build(MathCommonAlg.randomFloat(0.8f, 1f), -.2f);//StaticValue.build(0.5f);//
		caculateCommonHandle.c_alpha =StandarCal.build(255, -40);
		bitmapShape.setCaculateCommonHandle(caculateCommonHandle);
//		bitmapShape.setPeriod(2000);
//		bitmapShape.setBlank(1000);
		bitmapShape.setStartOffset((int) (startOffset + 1000 * valueF));
	}
...
//		caculateCommonHandle.c_scale = StaticValue.build(0.6f);//
	...
	*/
	public static void endowLevel10DownLight(BitmapShape light,Rect rect,int offsetTime,int duration){
		...
		final int width = rect.width();
		final int height = rect.height();
		float bitmapHeight=light.getBitmapHeight();
		...
		caculateCommonHandle.c_y =Decay2.build(height*6,.7f,0);//MulSimpleCal.build(new int[]{duration,200,400}, new float[]{bottom,height/2,height/2},0,MulEndMode.WITH_END_CAL);//DecayOpposite2.build(index, ratio);//(index, ratio, last)//MulSimpleCal.build(new int[]{duration}, new float[]{bottom+MathCommonAlg.rangeRandom(height/2,height)},-bitmapHeight,MulEndMode.WITH_END_CAL);//RangeCommon.build(0,top+MathCommonAlg.rangeRandom(0,height/2),duration);//StandarCal.build(0, MathCommonAlg.rangeRandom(400, 800));
		caculateCommonHandle.c_rotation = StaticValue.build(0);
		caculateCommonHandle.c_scale = StaticValue.build(1);
		caculateCommonHandle.c_alpha =MulSimpleCal.build(new int[]{duration,200},new float[]{0,-50},255,MulEndMode.WITH_END_CAL);
		light.setCaculateCommonHandle(caculateCommonHandle);
		int blank=MathCommonAlg.rangeRandom(0,50);
		light.setBlank(blank);
		light.setPeriod(duration+300);
		light.setDuration((duration+300)*2 + blank);
	 ...
	 * @return: void
	*/
	public static void endowLevel10DownLight2(BitmapShape light,Rect rect,int offsetTime,int duration){
		CaculateCommonHandle caculateCommonHandle = new CaculateCommonHandle();
		final int left = rect.left;
		final int top = rect.top;
		final int right = rect.right;
		...
		duration=duration+MathCommonAlg.rangeRandom(0, 1000);
		caculateCommonHandle.c_x =StaticValue.build(MathCommonAlg.rangeRandom(rect.left, rect.right));
		caculateCommonHandle.c_y =MulSimpleCal.build(new int[]{duration}, new float[]{bottom+height+MathCommonAlg.rangeRandom(0,height/2)},-bitmapHeight,MulEndMode.WITH_END_VALUE);//RangeCommon.build(0,top+MathCommonAlg.rangeRandom(0,height/2),duration);//StandarCal.build(0, MathCommonAlg.rangeRandom(400, 800));
		caculateCommonHandle.c_rotation = StaticValue.build(0);
		caculateCommonHandle.c_scale =StaticValue.build(MathCommonAlg.randomFloat(.8f, 1));//MulSimpleCal.build(new int[]{duration,200,200,200,200,200},new float[]{0,-.5f,.5f,-.5f,.5f,-.8f},1,MulEndMode.WITH_END_VALUE);
		caculateCommonHandle.c_alpha =MulSimpleCal.build(new int[]{duration,500,500,500,500,1000},new float[]{0,-150,150,-150,150,-255},255,MulEndMode.WITH_END_CAL);
		light.setCaculateCommonHandle(caculateCommonHandle);	
//		int blank=MathCommonAlg.rangeRandom(0,20);
////		light.setBlank(blank);
//		light.setPeriod(duration+3000);
//		light.setDuration((duration+3000)*2);
		light.setStartOffset(offsetTime+MathCommonAlg.rangeRandom(0, 3000));
	}
	/**
	 * level10中星云测试
	 * @param rect
	...
	*/
	public static void endowLevel10RowLight(BitmapShape light,Rect rect,int startOffsetTime,int duration){
		...
		caculateCommonHandle.c_y =RangeCommon.build(top-height/2, top+height/2, duration);//MulSimpleCal.build(new int[]{1000,1000,500},new float[]{-height,height,-height/2},bottom-height/2,MulEndMode.WITH_END_VALUE);//RangeCommon.build(, top-height/2, duration);
		caculateCommonHandle.c_rotation = StaticValue.build(0);
...
//		caculateCommonHandle.c_scale_x = MulSimpleCal.build(new int[]{2500, 500},new float[]{0,-1},1,MulEndMode.WITH_END_VALUE);
	 ...
	 * @return: void
	*/
	public static void endowLevel10ColLight(BitmapShape light,Rect rect,int startOffsetTime,int duration){
		CaculateCommonHandle caculateCommonHandle = new CaculateCommonHandle();
		final int left = rect.left;
		final int top = rect.top;
		final int right = rect.right;
...
//		caculateCommonHandle.c_scale_y = MulSimpleCal.build(new int[]{2500, 400},new float[]{0,-1},1,MulEndMode.WITH_END_VALUE);
//		caculateCommonHandle.c_scale_x = MulSimpleCal.build(new int[]{2500, 500},new float[]{0,-1},1,MulEndMode.WITH_END_VALUE);
		caculateCommonHandle.c_alpha =MulSimpleCal.build(new int[]{duration/4,duration/2,duration/4},new float[]{255,0,-255},0,MulEndMode.WITH_END_VALUE);
		light.setCaculateCommonHandle(caculateCommonHandle);
		light.setDuration(duration);
		light.setStartOffset(startOffsetTime);
	}
	...
	*/
	 ...
	 * @return: void
	*/
	public static void endowLevel10BGLight(BitmapShape element,BitmapShape element2,Rect rect,int startOffset,int duration,Context context) {
		final float bitmapWidth=element.getBitmapWidth();
		CaculateCommonHandle caculateCommonHandle = new CaculateCommonHandle();
		caculateCommonHandle.c_x =RangeCommon.build(rect.right,rect.left,4000);
		caculateCommonHandle.c_y = StaticValue.build(rect.top);
		caculateCommonHandle.c_rotation = StaticValue.build(0);
		caculateCommonHandle.c_scale = StaticValue.build(1);
		caculateCommonHandle.c_alpha = TimeLoop2.build(1, 255, 0.6f, 0.8f);//StaticValue.build(200);
		element.setCaculateCommonHandle(caculateCommonHandle);
		element.setStartOffset(startOffset);
		element.setDuration(duration);
		caculateCommonHandle = new CaculateCommonHandle();
		caculateCommonHandle.c_x =RangeCommon.build(rect.right-PXUtils.dp2px(context,170),rect.left-PXUtils.dp2px(context,170), 4000);
		caculateCommonHandle.c_y = StaticValue.build(rect.top-PXUtils.dp2px(context, 20));
		caculateCommonHandle.c_rotation = StaticValue.build(0);
		caculateCommonHandle.c_scale = StaticValue.build(1);
		caculateCommonHandle.c_alpha = StaticValue.build(255);//TimeLoop2.build(1, 255, 0.6f, 0.8f);//StaticValue.build(200);
		element2.setCaculateCommonHandle(caculateCommonHandle);
		element2.setStartOffset(startOffset);
		element2.setDuration(duration);
	}
	/**
	 * level10中信息面板的一个hengxiang paoguan
	 * @param element
	 * @param rect
	 * @param startOffset
	 * @param duration
	 * @return: void
	*/
	public static void endowLevel10BGRowLight(BitmapShape element,Rect rect,int startOffset,int duration) {
		final float bitmapWidth=element.getBitmapWidth();
		CaculateCommonHandle caculateCommonHandle = new CaculateCommonHandle();
		caculateCommonHandle.c_x =RangeCommon.build(rect.right-bitmapWidth/3*2,rect.right-bitmapWidth, 2000);
     ...
     * @return: void
     */
    public static void endowOpenGuardFlower(BitmapShape flower, int height, int width, int total, int offsetTime, int duration) {
        int blank = MathCommonAlg.rangeRandom(0, 50);
        CaculateCommonHandle caculateCommonHandle = new CaculateCommonHandle();
        float bitmapHeight = flower.getBitmapHeight();
        duration = duration + MathCommonAlg.rangeRandom(0, 1000);
        caculateCommonHandle.c_x = RightValue.build(-MathCommonAlg.rangeRandom(0, 350));
        caculateCommonHandle.c_y = new Decay2Stop(height >> 2, .7f, 0, total);// MulSimpleCal.build(new int[]{duration,200,400}, new
                                                                     // float[]{bottom,height/2,height/2},0,MulEndMode.WITH_END_CAL);//DecayOpposite2.build(index,
                                                                     // ratio);//(index, ratio, last)//MulSimpleCal.build(new
                                                                     // int[]{duration}, new
                                                                     // float[]{bottom+MathCommonAlg.rangeRandom(height/2,height)},-bitmapHeight,MulEndMode.WITH_END_CAL);//RangeCommon.build(0,top+MathCommonAlg.rangeRandom(0,height/2),duration);//StandarCal.build(0,
                                                                     // MathCommonAlg.rangeRandom(400, 800));
        float direction;
        if (MathCommonAlg.randomBool()) {
            // caculateCommonHandle.c_x = DecayOpposite2.build(MathCommonAlg.randomFloat(-1, 0), bitmap.getWidth() + bitmap.getWidth() / 2);
            direction = MathCommonAlg.randomFloat(-1, 0);
        } else {
            direction = MathCommonAlg.randomFloat(0, 1);
            // caculateCommonHandle.c_x = Decay2.build(width / 2 * direction, MathCommonAlg.randomFloat(0.1f, 2), left + bitmap.getWidth() /
            // 2);
        }
        caculateCommonHandle.c_rotation = MulSimpleCal.build(new int[] { 1000, 1000 }, new float[] { 180 * direction, 180 * direction }, 0,
                EnumConStant.MulEndMode.WITH_END_CAL);// StaticValue.build(0);
        caculateCommonHandle.c_scale = StaticValue.build(MathCommonAlg.randomFloat(0.5f, 1f));
        caculateCommonHandle.c_alpha = MulSimpleCal.build(new int[] { duration, duration }, new float[] { -125, -125 }, 255, MulEndMode.WITH_END_CAL);
        
        flower.setCaculateCommonHandle(caculateCommonHandle);
        flower.setBlank(blank);
        flower.setPeriod(duration + 300);
        flower.setDuration((duration + 300) * 2 + blank);
        flower.setStartOffset(MathCommonAlg.rangeRandom(0, 1000));
    }
     ...
     * @return: void
    */
    public static void endowOpenGuardRedHeart(BitmapShape b1, BitmapShape b2, BitmapShape b3, Rect rect, int timeOffset, float scale, float bitmapWidth,
            float bitmapHeight) {
        final int last = 1500;// 每个动画时间
        final int blankTime = 4000;
        final float centerX = rect.left - bitmapWidth / 2;
        ...
        caculateCommonHandle = new CaculateCommonHandle();
        caculateCommonHandle.c_x = MulSimpleCal.build(new int[] { last }, new float[] { rect.width() / 2 }, rect.left - bitmapWidth / 2 + rect.width() / 4,
                EnumConStant.MulEndMode.WITH_END_VALUE);// StaticValue.build(rect.width()/2f+rect.left-bitmapWidth/2);
        caculateCommonHandle.c_y = MulSimpleCal.build(new int[] { last }, new float[] { -rect.height() * 0.75f }, rect.bottom - bitmapHeight / 2,
                EnumConStant.MulEndMode.WITH_END_VALUE);
        float value = (float) Math.toDegrees(Math.atan((rect.width() / 2) / (rect.height() * 0.75f)));
        caculateCommonHandle.c_rotation = StaticValue.build(value);
        ...
        caculateCommonHandle = new CaculateCommonHandle();
        caculateCommonHandle.c_x = MulSimpleCal.build(new int[] { last }, new float[] { -rect.width() / 2 }, rect.width() / 2f + rect.left - bitmapWidth / 2,
                EnumConStant.MulEndMode.WITH_END_VALUE);
        caculateCommonHandle.c_y = MulSimpleCal.build(new int[] { last }, new float[] { -rect.height() * 0.55f }, rect.bottom - bitmapHeight / 2,
                EnumConStant.MulEndMode.WITH_END_VALUE);// MulSimpleCal.build(new int[]{last},new
                                                        // float[]{-rect.height()+bitmapHeight},rect.bottom-bitmapHeight/2,
                                                        // EnumConStant.MulEndMode.WITH_END_VALUE);
        value = (float) Math.toDegrees(Math.atan((rect.width() / 2) / (rect.height() * -0.55f)));
        caculateCommonHandle.c_rotation = StaticValue.build(value);
        caculateCommonHandle.initComScale(0f, 0f, scale, scale);//
     ...
     * @return: void
    */
    public static void endowOpenGuardbg(Element element, float startX, float startY, int startOffsetTime, int duration) {
        CaculateCommonHandle caculateCommonHandle = new CaculateCommonHandle();
        caculateCommonHandle.c_x = StaticValue.build(startX);
        caculateCommonHandle.c_y = StaticValue.build(startY);
        caculateCommonHandle.c_rotation = StaticValue.build(0);
        caculateCommonHandle.c_scale = StaticValue.build(1);
        caculateCommonHandle.c_alpha = TimeAlpha.build(0, 1500);
        element.setCaculateCommonHandle(caculateCommonHandle);
        element.setStartOffset(startOffsetTime);
        element.setDuration(duration);
    }
}
